...
CREATE TABLE stadiums(
    ...
    capacity INTEGER CHECK ( capacity > 0 ) NOT NULL,
        town_id INTEGER REFERENCES towns(id) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL
);
...
CREATE TABLE players_coaches(
    ...
    coach_id INTEGER REFERENCES coaches(id) ON DELETE CASCADE ON UPDATE CASCADE
);